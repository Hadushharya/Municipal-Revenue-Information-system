...
if ($id !== '') {
    $where .= " AND REPLACE(id, ' ', '') = ?";
    $params[] = str_replace(' ', '', $id);
    $types .= "s";
}

if ($fullname !== '') {
    $where .= " AND REPLACE(payer_name, ' ', '') LIKE ?";
    $params[] = "%" . str_replace(' ', '', $fullname) . "%";
    $types .= "s";
}

if ($id === '' && $fullname === '') {
    $message = "በጃኩም መለለይ ቁፅሪ ወይድማ ሙሉእ ሽም የእትዉ.";
} else {
    ...
    if ($count_stmt) {
        ...
        if ($total_rows > 0) {
            ...
            $sql = "SELECT client_id, payer_name, payer_contact, id, address FROM clients $where ORDER BY client_id DESC LIMIT ? OFFSET ?";
        ...
        $message = "Database error: Could not prepare count statement.";
    }
}
?>
...
<fieldset>
    <legend>ተጠቀምቲ መለለይ ቁፅሪ ወይ ሽም ምድላይ</legend>
    <form method="GET" action="">
        <label for="id">መለለይ ቁፅሪ:</label>
        <input type="text" name="id" id="id" placeholder="መለለይ ቁፅሪ" value="<?= htmlspecialchars($id ?? '') ?>" />

        <label for="fullname">ሽም ምስ ኣባሓጎ:</label>
        <input type="text" name="fullname" id="fullname" placeholder="ሽም ምስ ኣባሓጎ" value="<?= htmlspecialchars($fullname ?? '') ?>" />
    ...
    <div class="record-box">
        <table>
            ...
            <?php foreach ($records as $i => $row): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($row['payer_contact']) ?></td>
                    <td><?= nl2br(htmlspecialchars($row['address'])) ?></td>
                    <td>
                        <a class="blue" href="decidingtaxotherclientdetails?id=<?= urlencode($row['client_id']) ?>" title="ናብ ዝተዛወረ መረጃ">
        ...
        <div class="pagination">
            <?php
                $query_params = [];
                if (!empty($id)) $query_params['id'] = $id;
                if (!empty($fullname)) $query_params['fullname'] = $fullname;
