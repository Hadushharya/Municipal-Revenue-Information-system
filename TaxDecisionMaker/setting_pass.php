...
<head>
		...
		<div class="main-content">
			...
			if ($result->num_rows > 0) {
				...
				$currentHashedPassword = $row1['password'];
			}
			...
			if (isset($_POST['save'])) {
				...
				if (password_verify($Current, $currentHashedPassword)) {

					if ($New === $Confirm) {
						// Validate password strength
						if (preg_match('#^(?=.*[A-Za-z])(?=.*\d)(?=.*\W).{8,}$#', $New)) {

							// Check if same password is already used by another user
							...
							if ($passwordUsed) {
								$error = "<p class='red'>New password is already used by another user. Please choose a different one.</p>";
							} else {
								...
								$stmt->bind_param("ss", $newHashedPassword, $user1);
								if ($stmt->execute()) {
									$error = "<p class='green'>Password changed successfully.</p>";
								} else {
									$error = "<p class='red'>Error updating password. Please try again.</p>";
								}
							}

						} else {
							$error = "<p class='red'>Password must be Minimam 8 characters, include at least one letter, one number, and one symbol.</p>";
						}

					} else {
						$error = "<p class='red'>New Password and Confirmation Password do not match.</p>";
					}
