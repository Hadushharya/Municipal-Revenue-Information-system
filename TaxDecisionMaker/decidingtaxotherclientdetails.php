...
$clientid = $payer_name = $payer_contact = $address = '';

// Fetch client data
$sql = "SELECT client_id, payer_name, payer_contact, id, address FROM clients WHERE client_id = ?";
...
if ($row = $result->fetch_assoc()) {
    ...
    $payer_name = htmlspecialchars($row['payer_name'], ENT_QUOTES, 'UTF-8');
	$cid = htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8');
    $payer_contact = htmlspecialchars($row['payer_contact'], ENT_QUOTES, 'UTF-8');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    $client_id     = filter_input(INPUT_POST, 'clientid', FILTER_VALIDATE_INT);
    $payment_type  = filter_input(INPUT_POST, 'payment_type', FILTER_SANITIZE_STRING);
    $amount        = filter_input(INPUT_POST, 'amount', FILTER_VALIDATE_FLOAT);
    $registered_at  = filter_input(INPUT_POST, 'ethiopianDate1', FILTER_SANITIZE_STRING);
    $status = 'notapproved'; // default hardcoded, not from user input
    $remarks       = filter_input(INPUT_POST, 'remarks', FILTER_SANITIZE_STRING);
    //$registered_by = 1; // Replace with session user ID or logged in user
    $payment_date='';
	$paymentby='';
	$tin='';
	$receipt_number='';
	$paidby='';
    if ($client_id && $payment_type && $amount && $registered_at && $status) {
        $sql = "INSERT INTO `other_payments`(`client_id`, `payment_type`, `amount`, `payment_date`, `status`, `paymentby`, `tin`, `receipt_number`, `registered_by`, `registered_at`,`paidby`) 
            VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("isdssssisss", 
                 $client_id, $payment_type, $amount, $payment_date, $status, $paymentby,$tin,$receipt_number, $userfullname,$registered_at,$paidby
            );

            if ($stmt->execute()) {
                $message = '<p class="success" style="color:green;">✅ ክፍሊት ብትክክል ተፈፂሙሎም ኣሎ::.</p>';
				
            } else {
                $message = '<p class="error" style="color:red;">❌ Error executing statement: ' . $stmt->error . '</p>';
            }

            $stmt->close();
        } else {
            $message = '<p class="error" style="color:red;">❌ Database error: ' . $conn->error . '</p>';
        }
    } else {
        $message = '<p class="error" style="color:red;">❌ እቶም ዘድልዩ ፊልድታት ኩሎም ብትክክል ኣይተመልኡን.</p>';
    }
}
?>
...
<form class="form-horizontal" name="dateForm" role="form" method="post" enctype="multipart/form-data">
    <fieldset>
        ...
        <!-- Client Info Display -->
        <div class="form-group">
            <label class="col-sm-2 control-label no-padding-right" for="clientid">መለለይ ቁፅሪ:</label>
            <div class="col-sm-10">
        ...
        <div class="form-group">
            <label class="col-sm-2 control-label no-padding-right" for="payer_name">ሙሉእ ሽም:</label>
            <div class="col-sm-10">
                <input type="text" id="payer_name" name="payer_name" 
                       value="<?php echo $payer_name ?? ''; ?>" 
                       readonly class="col-xs-10 col-sm-5" />
            </div>
        </div>

        <div class="form-group">
            <label class="col-sm-2 control-label no-padding-right" for="payer_contact">ስልኪ ቁፅሪ:</label>
            <div class="col-sm-10">
                <input type="text" id="payer_contact" name="payer_contact" 
                       value="<?php echo $payer_contact ?? ''; ?>" 
                       readonly class="col-xs-10 col-sm-5" />
            </div>
        </div>

        <div class="form-group">
            <label class="col-sm-2 control-label no-padding-right" for="address">ኣድራሻ (Address):</label>
            <div class="col-sm-10">
        ...
        <!-- Additional Fields for other_payments -->
        <div class="form-group">
            <label class="col-sm-2 control-label" for="payment_type">ዓይነት ክፍሊት:</label>
            <div class="col-sm-10">
                <select name="payment_type" id="payment_type" class="col-xs-10 col-sm-5" required>
                    <option value="">-- ምረፅ --</option>
                    <option value="ትራፊክ">ትራፊክ</option>
					<option value="ሰነድ">ሰነድ</option>
                    <option value="ፕላን">ፕላን</option>
                    <option value="ሽካል">ሽካል</option>
					 <option value="ካልኦት">ካልኦት</option>
                </select>
            </div>
        </div>

        <div class="form-group">
            <label class="col-sm-2 control-label" for="amount">መጠን ክፍሊት:</label>
            <div class="col-sm-10">
                <input type="number" step="0.01" name="amount" id="amount" required 
                       class="col-xs-10 col-sm-5" placeholder="መጠን ክፍሊት" />
            </div>
        </div>
    ...
    <thead>
        <tr>
            ...
            <th class='blue'>ዓይነት ክፍሊት</th>
            <th class='blue'>መጠን (ብር)</th>
            
           
            <th class='blue'>ባዓል ሞያ ኣወሳስና</th>
            <th class='blue'>ዝተመዝገበሉ ዕለት</th>
    ...
    <tbody>
<?php
// Prepare and fetch records from other_payments
$q = "SELECT p.*, c.payer_name 
      FROM other_payments p
...
while ($row = $res->fetch_assoc()) {
    ...
    echo "<td>" . htmlspecialchars($row['payer_name'], ENT_QUOTES, 'UTF-8') . "</td>";
    echo "<td>" . htmlspecialchars($row['payment_type'], ENT_QUOTES, 'UTF-8') . "</td>";
    echo "<td>" . number_format((float)$row['amount'], 2) . "</td>";
    echo "<td>" . htmlspecialchars($row['registered_by'], ENT_QUOTES, 'UTF-8') . "</td>";
    echo "<td>" . htmlspecialchars($row['registered_at'], ENT_QUOTES, 'UTF-8') . "</td>";
    echo "<td></td>";
...
if ($i === 0) {
    echo "<tr><td colspan='13' style='text-align:center;'>No payments found.</td></tr>";
}
?>
    </tbody>
</table>
