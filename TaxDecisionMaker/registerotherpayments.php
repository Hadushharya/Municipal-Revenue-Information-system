...
if (isset($_POST['save'])) {
    ...
    $payer_name = htmlspecialchars(trim($_POST['payer_name']), ENT_QUOTES, 'UTF-8');
$payer_contact = htmlspecialchars(trim($_POST['payer_contact']), ENT_QUOTES, 'UTF-8');
$payer_id = htmlspecialchars(trim($_POST['id']), ENT_QUOTES, 'UTF-8');
$address = htmlspecialchars(trim($_POST['address']), ENT_QUOTES, 'UTF-8');


    // Prepare the INSERT statement
    $stmt = mysqli_prepare($conn, "INSERT INTO clients (payer_name, payer_contact, id, address) VALUES (?, ?, ?, ?)");

    if ($stmt) {
        // Bind parameters
        mysqli_stmt_bind_param($stmt, "ssss", $payer_name, $payer_contact, $payer_id, $address);

        // Execute the query
        if (mysqli_stmt_execute($stmt)) {
            echo '<p class="success" style="color:#390">Client successfully registered.</p>';
            //echo '<meta http-equiv="refresh" content="3; url=registerotherpayments.php">';
        } else {
            echo '<p style="color:red">Error: Could not register client. Maybe already exists?</p>';
        }
...
<form class="form-horizontal" name="dateForm" role="form" method="post" enctype="multipart/form-data">
  <fieldset>
    ...
    <div class="form-group">
      <label for="fullname">ሽም ምስ ኣባሓጎ:</label>
      <input type="text" name="payer_name" id="fullname" placeholder="ሽም ምስ ኣባሓጎ" required />
    </div>

    <!-- Phone Number -->
    <div class="form-group">
      <label for="phone">ስልኪ ቁፅሪ:</label>
      <input type="text" name="payer_contact" id="phone" placeholder="ስልኪ ቁፅሪ" required />
    </div>

    <!-- ID -->
    <div class="form-group">
      <label for="id">መለለይ ቁፅሪ:</label>
      <input type="text" name="id" id="id" placeholder="መለለይ ቁፅሪ" />
    </div>

    <!-- Address -->
    <div class="form-group">
      <label for="address">ኣድራሻ:</label>
      <textarea name="address" id="address" rows="3" placeholder="ኣድራሻ"></textarea>
    </div>

    <!-- Submit & Reset Buttons -->
    <div class="form-actions">
      <button type="submit" name="save" class="btn btn-primary">
        <i class="fa fa-plus"></i> መዝግብ
      </button>
      <button type="reset" class="btn btn-secondary">
        <i class="fa fa-times"></i> ኣፅሪ
      </button>
    </div>
  </fieldset>
</form>
