<?php
// receiptpreparationotherpayments.php
...
function numberToTigrinya($number) {
    $t = function($n, $rec) use (&$t) {
      ...
      if ($n < 100) {
				$tens = intval($n / 10) * 10;
				$units = $n % 10;
				return $words[$tens] . ($units ? "ን " . $words[$units] . "ን" : "ን");
			}
...
if (!isset($_GET['clientid']) || !intval($_GET['clientid'])) {
...
// ========== lookup rent account ==========
$rentAccCode = '1706';
$rentAccDesc = 'ካልኦት ኣታዊታት';
$qq = $conn->query("SELECT acc_cod, income_type FROM codes WHERE acc_cod='1706' LIMIT 1");
if ($qq && $qq->num_rows) {
    $r = $qq->fetch_assoc();
...
function getClientName($conn, $client_id) {
    $name = '';
    $st = $conn->prepare("SELECT payer_name FROM clients WHERE client_id = ?");
    if ($st) {
        ...
        <div class="page-content">
            ...
            <div class="col-xs-12">
                ...
                <?php if (empty($grouped)): ?>
                    <p>ዝተመረፁ ክፍሊታት ኣብ approved ኣይተረኽበን።</p>
                <?php else: ?>
                    <?php foreach ($grouped as $client_id => $dates): ?>
                        <?php $clientName = getClientName($conn, $client_id); ?>
                        <?php foreach ($dates as $payment_date => $records): ?>
                            <?php
                                ...
                                foreach ($records as $row) {
                                    if (!empty($row['payment_type'])) $typesArr[] = $row['payment_type'];
                                    if (!empty($row['receipt_no'])) $receipts[] = $row['receipt_no'];
                                    if (!empty($row['paidby'])) $paidBy = $row['paidby'];
                                    $grandTotal += (float)$row['amount_paid'];
                                }
                                $typesStr = implode(', ', array_unique($typesArr));
                                $receiptStr = implode(', ', array_unique($receipts));
                                $paymentDateDisplay = htmlspecialchars($payment_date);
                            ?>
                            <div id="printableDiv" class="panel-body"><br>
                                <form class="form-horizontal" role="form">
                                    <fieldset>
                                        <legend><u><b>ናይ ክፍሊት ቅጥዒ</b></u></legend>
                                        <p align="right">ደረሰኝ ቁፅሪ: <b><?= $receiptStr ?: '—' ?></b></p>
                                        <p align="right">ዕለት: <b><?= $paymentDateDisplay ?></b></p>
                                        <p>ሙሉእ ሽም ተኸታታሊ: <b><?= htmlspecialchars($clientName) ?></b></p>
                                        <p>ዓይነት ክፍሊት: <b><?= htmlspecialchars($typesStr) ?></b></p>
                                    </fieldset>
                                    ...
                                    <table border="2" class="table table-hover table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><?= htmlspecialchars($rentAccCode) ?></td>
                                                <td><?= htmlspecialchars($rentAccDesc) ?></td>
                                                <td>0.00</td>
                                                <td><?= number_format($grandTotal, 2) ?></td>
                                            </tr>
                                        </tbody>
                                    ...
                                    <p>ብፊደል (In words): <b><?= htmlspecialchars(numberToTigrinya($grandTotal)) ?></b></p>
                                    <p>ስም ተቐባሊ ገንዘብ: <b><?= htmlspecialchars($paidBy ?: $username) ?></b></p>
