...
<script>
 ...
 // Display Ethiopian date (no time)
 function displayEthiopianDate() {
    ...
    // Only date, no time
    const dateString = `${ethDate.day}/${ethDate.month}/${ethDate.year}`;

    // Update the page with Ethiopian date only
    document.getElementById("ethiopianDate").textContent = dateString;
    document.getElementById("ethiopianDate1").value = dateString;
 ...
 // Display Ethiopian date in a specific section (no time)
 function getToday() {
    ...
    todayElement.innerHTML = `${dateString} ${getInWords()} E.C.<br/>`;
 }
...
<?php
// session_start();
// require_once 'db_connection.php'; // Ensure you include DB connection
...
// Get user full name
$userfullname = '';
$stmtUser = $conn->prepare("SELECT fname, mname, lname FROM users WHERE id = ?");
$stmtUser->bind_param("i", $id1);
$stmtUser->execute();
$resultUser = $stmtUser->get_result();
if ($userRow = $resultUser->fetch_assoc()) {
    $userfullname = trim("{$userRow['fname']} {$userRow['mname']} {$userRow['lname']}");
}
$stmtUser->close();
...
if (!empty($ids) && $clientId > 0) {
    // Fetch payer info
    $stmtClient = $conn->prepare("SELECT id, payer_name FROM clients WHERE client_id = ?");
    $stmtClient->bind_param("i", $clientId);
    $stmtClient->execute();
    $clientResult = $stmtClient->get_result();
    if ($clientRow = $clientResult->fetch_assoc()) {
        $payerName = trim("{$clientRow['payer_name']}");
        $clientCode = $clientRow['id'];
    }
    $stmtClient->close();

    // Fetch grouped payment info
    $placeholders = implode(',', array_fill(0, count($ids), '?'));
    $sql = "
        SELECT payment_type, amount, registered_at, paidby
        FROM other_payments
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $groupedPayments[$type] = isset($groupedPayments[$type]) ? $groupedPayments[$type] + $row['amount'] : $row['amount'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $ethiopianDate = $_POST['ethiopianDate1'] ?? '';
    $status        = 'approved';
    //$registered_by = $userfullname;
    $paidby_update = $paidby;
    ...
    if (!empty($ids)) {
        ...
        $stmtUpdate->close();
        echo "<div class='alert alert-success'>ክፍሊት ብትክክል ተፈፂሙ ኣሎ.</div>";
    }
}
?>

<!-- PAYMENT FORM -->
<form name="otherPaymentForm" class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
    <fieldset>
        <legend>ናይ ካልኦት ክፍሊታት ምፍፃም</legend>
        ...
        <!-- Client Info -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ሙሉእ ሽም ካፋሊ/ት:</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars($payerName) ?>" />
            </div>
        </div>

        <div class="form-group">
            <label class="col-sm-2 control-label">መለለዪ ቁፅሪ (ID):</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars($clientCode) ?>" />
            </div>
        </div>

        <!-- Registered Date -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ዝመዝገበሉ ዕለት:</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars($registered_at) ?>" />
            </div>
        </div>

        <!-- Paid By (previous) -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ተኸፊላይ ዝተመዝገበ:</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars($paidby) ?>" />
            </div>
        </div>

        <!-- Payment Types and Amounts -->
        <?php foreach ($groupedPayments as $type => $amount): ?>
        <div class="form-group">
            <label class="col-sm-2 control-label"><?= htmlspecialchars($type) ?>:</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars(number_format($amount, 2)) ?>" />
            </div>
        </div>
        <?php endforeach; ?>
        ...
        <!-- Total Amount -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ድምር መጠን ገንዘብ:</label>
            <div class="col-sm-4">
                <input type="text" readonly class="form-control" value="<?= htmlspecialchars(number_format($totalAmount, 2)) ?>" />
            </div>
        </div>

        <!-- PAYMENT METHOD -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ክፍሊት ዝፍፀም:</label>
            <div class="col-sm-4">
                <select name="paymentby" id="paymentby" class="form-control" required>
        ...
        <div class="form-group" id="payment-group" style="display:none;">
            <label class="col-sm-2 control-label">TIN ቁፅሪ:</label>
            <div class="col-sm-4">
                <input type="text" name="tin" id="tin" class="form-control" placeholder="TIN ቁፅሪ" />
            </div>
        </div>

        <!-- Receipt Number -->
        <div class="form-group">
            <label class="col-sm-2 control-label">ደረሰኝ ቁፅሪ:</label>
            <div class="col-sm-4">
        ...
        <div class="form-group">
            <div class="col-sm-offset-2 col-sm-10">
                <button type="submit" name="save" class="btn btn-primary">ክፍሊት ኣኸፍል</button>
                <button type="reset" class="btn btn-default">ኣጥፋ</button>
            </div>
        </div>
    </fieldset>
</form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var tinGroup = document.getElementById('payment-group');
    var tinInput = document.getElementById('tin');

    paymentBy.addEventListener('change', function () {
        if (this.value === 'ባንኪ') {
            tinGroup.style.display = 'block';
        } else {
            tinGroup.style.display = 'none';
            tinInput.value = '';
        }
    });
});
