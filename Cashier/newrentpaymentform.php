...
$i = 0;
//$status = 'notapproved';
$status = filter_var('notapproved', FILTER_SANITIZE_STRING);
// Fetch all payments ordered by homeowner_id and payment_date
$q = "
    SELECT *
    FROM home_rent_payments
    WHERE status = ?
    ORDER BY homeowner_id ASC, payment_date ASC, payment_id ASC
";
...
// Group by homeowner_id AND payment_date
$grouped = [];
while ($row = $result->fetch_assoc()) {
    $homeowner_id = (int)$row['homeowner_id'];
    $paymentDate = $row['payment_date']; // normalize date
    $grouped[$homeowner_id][$paymentDate][] = $row;
}

// Display grouped records
foreach ($grouped as $homeowner_id => $dates) {
    // Fetch homeowner fullname & room_cost once
    $stmtHome = $conn->prepare("SELECT name, room_cost FROM homeowners WHERE homeowner_id = ?");
    $stmtHome->bind_param("i", $homeowner_id);
    $stmtHome->execute();
    $resHome = $stmtHome->get_result();
    if ($resHome->num_rows > 0) {
        $homeData = $resHome->fetch_assoc();
        $fullname_homeowner = $homeData['name'];
        $room_cost = $homeData['room_cost'];
    } else {
        $fullname_homeowner = '';
        $room_cost = '';
    }
    ...
    foreach ($dates as $paymentDate => $records) {
        ...
        $roomsArray = array_unique(array_column($records, 'rooms'));
        $rooms = implode(', ', $roomsArray);
      
        // Sum amounts
        $amountSum = array_sum(array_column($records, 'amount_paid'));
         $bankinterest = array_sum(array_column($records, 'bankinterest'));
		 $difference = array_sum(array_column($records, 'difference'));
		 $punishment = array_sum(array_column($records, 'punishment'));
		 $diffrencewithpunishment = array_sum(array_column($records, 'diffrencewithpunishment'));
        // Payment months (can repeat)
        $months = implode(', ', array_column($records, 'payment_month'));
         
        // Collect unique decidedby
        $decidedbyArray = array_unique(array_column($records, 'decidedby'));
        $decidedby = implode(', ', $decidedbyArray);
      // Collect unique receipt numbers
    $receiptArray = array_unique(array_column($records, 'receipt_no'));
    $receipt_no = implode(', ', $receiptArray);

        // IDs
        $ids = implode(',', array_column($records, 'payment_id'));

        echo "<tr>";
        echo "<td class='center'>{$i}</td>";
        echo "<td>" . htmlspecialchars($fullname_homeowner, ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td>" . htmlspecialchars($months, ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td>" . htmlspecialchars($rooms, ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td>" . htmlspecialchars($amountSum, ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($bankinterest, ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($difference, ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($punishment, ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($diffrencewithpunishment, ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td>" . htmlspecialchars(number_format($amountSum+$bankinterest+$punishment,2), ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($receipt_no, ENT_QUOTES, 'UTF-8') . "</td>";
		echo "<td>" . htmlspecialchars($paymentDate, ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td>" . htmlspecialchars($decidedby, ENT_QUOTES, 'UTF-8') . "</td>";

        // Action buttons
        echo "<td>
                <div class='hidden-sm hidden-xs action-buttons'>
                    <a class='blue' href='newrentpaymentform?id={$ids}&homeowner_id={$homeowner_id}' title='ክፍሊት ፈፅም'>
