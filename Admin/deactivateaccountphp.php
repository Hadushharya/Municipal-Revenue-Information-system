...
?>
    ...
    if ($stmt = mysqli_prepare($conn, $q)) {
        ...
        mysqli_stmt_bind_param($stmt, "i", $id); // "i" indicates the parameter is an integer
...
?>
								  ...
								  <label class="col-sm-2 control-label no-padding-right" for="form-field-1">ህልዊ ኩነታት:  </label>
										<select  class="col-xs-7 col-sm-1"  id="form-field-1"  required  name="availablitycondition" id="availablitycondition">
													...
													<option value="inactive"> ዘየለ/ላ</option>
...
$stmt = mysqli_prepare($conn, "UPDATE `users` SET `permission` = ? WHERE id = ?");
mysqli_stmt_bind_param($stmt, "si", $permission, $id);
...
if ($query) {
    echo "Account Successfully Deactivated ";
		//  Record log activity
			   ...
			   insertLog($conn, $useridad, $username, "Account Deactivated", "Account Deactivated Successfully.", "success", $ip, $agent);
