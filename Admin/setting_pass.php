...
require_once('../db/connection.php'); // Ensure $conn is defined

if (isset($_POST['save'])) {
    if (empty($_SESSION['SESS_USER_NAME'])) {
        $error = "<p class='red'>Session expired. Please log in again.</p>";
    } else {
        ...
        if (empty($Current) || empty($New) || empty($Confirm)) {
            $error = "<p class='red'>All fields are required.</p>";
        } elseif ($New !== $Confirm) {
            $error = "<p class='red'>New password and confirmation do not match.</p>";
        } elseif (!preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$/', $New)) {
            $error = "<p class='red'>Password must be at least 8 characters long and include letters, numbers, and symbols.</p>";
        } else {
            ...
            if ($stmt->num_rows > 0) {
                ...
                if (password_verify($Current, $hashed_password)) {
                    ...
                    $updateStmt = $conn->prepare("UPDATE admin SET password = ? WHERE user_name = ?");
                    $updateStmt->bind_param("ss", $new_hashed_password, $user);

                    if ($updateStmt->execute()) {
                        $error = "<p class='green'>Successfully changed password.</p>";
                    } else {
                        $error = "<p class='red'>Error updating password. Please try again later.</p>";
                    }

                    $updateStmt->close();
                } else {
                    $error = "<p class='red'>Your current password is incorrect.</p>";
                }
            } else {
                $error = "<p class='red'>User not found.</p>";
            }
