...
$query = "SELECT * FROM `clientslanddata` WHERE `id` = ? AND `forpayment_status` = 'approved'  ORDER BY `id` DESC";
...
?>
                             ...
                             <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
										...
										<div class="col-sm-10">
										   ...
										   <div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> ስልኪ ቁፅሪ:  </label>

										<div class="col-sm-10">
											<input type="text" id="form-field-1"  value='<?php echo $phone; ?>' name="phone"  required id="phone" placeholder="ስልኪ ቁፅሪ" class="col-xs-10 col-sm-5" />
									 	</div>
									    </div> 
                                 ...
                                 <div class="form-group">
								  ...
								  <label class="col-sm-2 control-label no-padding-right" for="form-field-1">ዓ/ግልጋሎት:  </label>
										<select  class="col-xs-9 col-sm-1"  id="form-field-1"  required  name="mainservice" id="mainservice">
													<option value=""> ምረፅ</option>
												<?php	   // Query to get distinct mainservice values
														$query = "SELECT * FROM rule";
														$result = mysqli_query($conn, $query);

														if ($result) {
															while ($row1 = mysqli_fetch_assoc($result)) {
																$mainservice = htmlspecialchars($row1['mainservice']); // safely encode
																echo "<option value=\"$mainservice\">$mainservice</option>";
															}
														}
														?>					
												</select>
...
if (isset($_POST['save'])) {
    ...
    $sql = "UPDATE `clientslanddata` 
            SET `fullname` = ?, `phone` = ?, `kebele` = ?, `block` = ?, `east` = ?, `west` = ?, `north` = ?, `south` = ?, 
                ...
                `forpayment_status` = 'approved', `siteplan` = ?, `buildingplan` = ?, `registeredtime` = CURRENT_TIMESTAMP 
            WHERE `forpayment_status` = 'approved' AND `id` = ?";
    ...
    if ($stmt) {
        ...
        mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssi", 
            $fullname,$phone, $kebele, $block, $east, $west, $north, $south, 
            $filenumber, $levelofplace, $occupiedyear, $mainservice, $kindofvolumeofland, $area, 
            $beginofconstruction, $endofconstruction, $beginpayment, $endpayment, $amountofpay, 
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo '<meta http-equiv="refresh" content="2;url=viewlandclient" />';
        } else {
