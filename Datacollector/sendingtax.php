...
//require 'db_connection.php'; // Adjust this to your actual DB connection script

// Define query with placeholder
$q = "SELECT * FROM `clientslanddata` WHERE forpayment_status = ? and area > ? ORDER BY id DESC";

// Prepare the statement
$stmt = mysqli_prepare($conn, $q);
if ($stmt) {
    $forpayment_status = 'notapproved'; // Define parameter value

    // Bind parameter (s = string)
    mysqli_stmt_bind_param($stmt, "sd", $forpayment_status,$area);
...
?>
													...
													<tr>
														...
														<td> 
													<?php if($row['forpayment_status']=="notapproved") { 
														
                        echo "<a href=approvesendingtax?id=".$id." class='btn red'>Approve</a>"; 
						
						//  Record log activity
						$ip = $_SERVER['REMOTE_ADDR'];
						$agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
					   insertLog($conn, $userid, $username, "Client Approval", "Client successfully Sent to Tax deciding", "success", $ip, $agent);
													}
						else{
							echo "error";
						//  Record log activity
						$ip = $_SERVER['REMOTE_ADDR'];
						$agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
					   insertLog($conn, $userid, $username, "Client Approval Failed", "Client approval Failed", "Fail", $ip, $agent);
						}				
											?> 
														</td>
															<td>
