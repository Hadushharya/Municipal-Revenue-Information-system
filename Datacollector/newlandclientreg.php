...
if (isset($_POST['save'])) { 
  ...
  // Define the status variable before binding it
    $status = 'notapproved';
 //$registeredtime = $ethiopianDate1;
 
    // Prepare the INSERT statement
    $stmt = mysqli_prepare($conn, "INSERT INTO `clientslanddata` 
        ...
        `forpayment_status`, `registeredby`, `registeredtime`) 
        VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?)");

    // Bind the parameters to the prepared statement
    mysqli_stmt_bind_param($stmt, "isssssssssssssiidiidsss", 
        $id1, $fullname,$phone, $kebele, $block, $east, $west, $north, $south, 
        ...
        $amountofpay, $status, $userfullname, $registeredtime);
    ...
    if (mysqli_stmt_execute($stmt)) {
	   ...
	   insertLog($conn, $userid, $username, "Client Registration", "Client successfully Registered", "success", $ip, $agent);
        ...
        echo ' <meta content="6;newlandregistration" http-equiv="refresh" />';
		 ...
		 $ip = $_SERVER['REMOTE_ADDR'];
        $agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
	 insertLog($conn, $userid, $username, "Client Registration", "Client Already Registered", "Fail", $ip, $agent);
        echo '<p style="color:red"> Error, Client Already Registered </p>';
...
?>
								...
								<h4>
								 <form class="form-horizontal" name="dateForm" role="form" method="post" enctype="multipart/form-data">
                                     ...
                                     <div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" id="form-field-1"  name="phone"  required id="phone" placeholder="ስልኪ ቁፅሪ" class="col-xs-10 col-sm-5" />
									 	</div>
									    </div> 
										 <div class="form-group">
								...
								<div class="form-group">
											  ...
											  <select class="col-xs-9 col-sm-1" id="mainservice" name="mainservice" required>
												<option value="">ምረፅ</option>

												<?php
											
												$query = "SELECT DISTINCT mainservice FROM rule WHERE mainservice IS NOT NULL AND mainservice != '' order by rule_id desc";
												$result = mysqli_query($conn, $query);

												
												while ($row = mysqli_fetch_assoc($result)) {
													$mainservice = htmlspecialchars($row['mainservice']);
										...
										<div class="col-sm-10">
										 ...
										 <div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> ክፍሊት መጀመሪ(ዓ/ም):  </label>
								...
								<!-- Land Type Selection -->
<div class="form-group">
  <label class="col-sm-2 control-label no-padding-right" for="kindofvolumeofland">ናይቲ መሬት ዓይነት:</label>
  <div class="col-sm-4">
    <select name="kindofvolumeofland" id="kindofvolumeofland" class="form-control col-xs-5 col-sm-3">
      <option value="">-- ምረፅ --</option>
...
<!-- For ሊዝ -->
<div id="liz-fields" style="display:none;">
  <div class="form-group">
    <label class="col-sm-2 control-label no-padding-right">ምእሳር ውዕል:</label><br/>
  </div>

  <div class="form-group">
    <label class="col-sm-3 control-label no-padding-right" for="beginofconstruction">ህንፀት መጀመሪ(ዓ/ም):</label>
    <div class="col-sm-9">
      <input type="text" name="beginofconstruction"  id="beginofconstruction" placeholder="ህንፀት መጀመሪ" class="col-xs-10 col-sm-5" />
    </div>
  </div>

  <div class="form-group">
    <label class="col-sm-3 control-label no-padding-right" for="endofconstruction">ህንፀት መወድኢ(ዓ/ም):</label>
    <div class="col-sm-9">
      <input type="text" name="endofconstruction"   id="endofconstruction" placeholder="ህንፀት መወድኢ" class="col-xs-10 col-sm-5" />
    </div>
  </div>
  <div class="form-group">
    <label class="col-sm-3 control-label no-padding-right" for="endpayment">ክፍሊት መወድኢ(ዓ/ም):</label>
    <div class="col-sm-9">
      <input type="text" name="endpayment"  id="endpayment" placeholder="ክፍሊት መወድኢ" class="col-xs-10 col-sm-5" />
    </div>
  </div>
</div>

<!-- Payment Amount Field (only for ሊዝ) -->
<div class="form-group" id="payment-group" style="display:none;">
  <label class="col-sm-2 control-label no-padding-right" for="amountofpay">ዝከፍሎ መጠን ገንዘብ(ብር):</label>
  <div class="col-sm-4">
    <input type="text" id="amountofpay" name="amountofpay" placeholder="መጠን ገንዘብ" class="form-control col-xs-5 col-sm-3" />
  </div>
</div>

<!-- jQuery -->
<script src="js/kindofvolumeoflanddropdown.js"></script>

<!-- Show/Hide Logic -->
<script>
  $(document).ready(function () {
    $('#kindofvolumeofland').on('change', function () {
      const val = $(this).val();
      
    if (val === 'ሊዝ') {
        $('#liz-fields, #payment-group').show();
        $('#kray-fields').hide();
      } else {
        $('#kray-fields, #liz-fields, #payment-group').hide();
      }
    });
  });
</script>
