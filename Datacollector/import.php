...
if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    if ($extension === "xlsx") {
        ...
        foreach ($rows as $rowNumber => $row) {
            ...
            $cleanRow = array_map(fn($v) => trim((string)($v ?? '')), array_values($row));
            while (count($cleanRow) < 26) {
                $cleanRow[] = '';
            }
            if (count($cleanRow) !== 26) {
                echo "Skipping row $rowNumber: column count = " . count($cleanRow) . "<br>";
                continue;
            }

            // Skip empty or duplicate filenumber
            if (empty($cleanRow[9])) {
                echo "Skipping row $rowNumber: filenumber is empty.<br>";
                continue;
            }
            $checkStmt = $conn->prepare("SELECT id FROM clientslanddata WHERE filenumber = ?");
            $checkStmt->bind_param("s", $cleanRow[9]);
            $checkStmt->execute();
            $checkStmt->store_result();
            if ($checkStmt->num_rows > 0) {
                echo "Skipping row $rowNumber: filenumber '{$cleanRow[9]}' already exists.<br>";
                $checkStmt->close();
                continue;
            }
            $checkStmt->close();
            ...
            $cleanRow[1] = (int) $cleanRow[1];
            $cleanRow[4] = (int) $cleanRow[4];
            $cleanRow[14] = (float) $cleanRow[14];
            $cleanRow[15] = (int) $cleanRow[15];
            $cleanRow[16] = (int) $cleanRow[16];
            $cleanRow[17] = (int) $cleanRow[17];
            $cleanRow[18] = (int) $cleanRow[18];
            $cleanRow[19] = (float) $cleanRow[19];
            $cleanRow[21] = (int) $cleanRow[21];
            $cleanRow[22] = (int) $cleanRow[22];

            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                die("Prepare failed: " . $conn->error);
            }
            $stmt->bind_param(str_repeat("s", 26), ...$cleanRow);
            if ($stmt->execute()) {
                $imported++;
            } else {
        ...
        echo " XLSX file processed. Rows inserted: $imported";

        $ip = $_SERVER['REMOTE_ADDR'];
        $agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
        insertLog($conn, $userid, $username, "Client Importing", "Lists of Clients Uploaded to the System", "success", $ip, $agent);
    }
    ...
    elseif ($extension === "csv") {
        ...
        echo " CSV file processed. Rows inserted: $imported";
        ...
        echo "Only XLSX and CSV files are supported.";
		     $ip = $_SERVER['REMOTE_ADDR'];
        $agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
        insertLog($conn, $userid, $username, "Trying to import", "To import Lists of Clients to the System, the files should be .csv or .xlsx", "Fail", $ip, $agent);
    }
    ...
    echo " No file uploaded or file error.";
	    $ip = $_SERVER['REMOTE_ADDR'];
        $agent = $_SERVER['HTTP_USER_AGENT'] ?? null;
        insertLog($conn, $userid, $username, "Trying to import", "No file uploaded or file error", "Fail", $ip, $agent);
}
?>
					</div>
								<div class="col-xs-12">
