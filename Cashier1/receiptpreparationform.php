   ...
   <head>
	...
	<body class="no-skin">
		...
		<div class="main-container ace-save-state" id="main-container">
			...
			<div class="main-content" onclick="window.print();" >
...
$idList = array_map('intval', $idList); // sanitize

if (!$userid || empty($idList)) die("User ID or payment IDs missing.");

// Prepare placeholders for IN clause
$placeholders = implode(',', array_fill(0, count($idList), '?'));

// SQL: join land tax and home rent payments on same client and same date
$sql = "
SELECT 
    c.clientid,
    c.userid,
    ...
    -- Only include rent payments for the same day
...
$params = array_merge([$userid], $idList);
...
?>
<style>
/* Hide everything except the printable div */
@media print {
    body * {
        visibility: hidden;
    }
    ...
    /* Force table borders and collapse for printing */
    #printableDiv table {
...
<?php while ($row = $result->fetch_assoc()): 
    // Merge land + rent years uniquely
    $landYears = array_filter(explode(',', $row['land_years'])); // remove empty
$rentYears = array_filter(explode(',', $row['rent_years']));

$allYears = array_unique(array_merge($landYears, $rentYears));
$yearsString = implode(',', $allYears);
?>
<div id="printableDiv" class="panel-body"><br>
    <form class="form-horizontal" role="form">
        <fieldset>
            <legend><u><b>ናይ ግብሪ ከፍላይ ቅብሊት ቅጥዒ</b></u> </legend>
            <p align='right'>ደረሰይ ቁፅሪ: <b><?php echo htmlspecialchars($row['land_receipt']); ?></b></p>
            <p align='right'>ዕለት: <b><?php echo htmlspecialchars($row['paidtime']); ?></b></p>
            <p>ሙሉእ ሽም ከፋሊ ግብሪ: <u><b><?php echo htmlspecialchars($row['fullname']); ?></b></u></p>
            <p>ከፋሊ ግብሪ ፋ/ቁፅሪ: <b><?php echo htmlspecialchars($row['filenumber']); ?></b></p>
             <p>ምክንያት ክፍሊት: <b><?php 
								if($row['sumTotal'] != 0 && $row['sumRent'] != 0){
									echo "ናይ ግብሪ መሬትን ክራይን መካረይን"; // Land + Rent
								}
								elseif($row['sumTotal'] != 0 && $row['sumRent'] == 0){
									echo "ናይ ግብሪ መሬት"; // Only Land
								}
								elseif($row['sumTotal'] == 0 && $row['sumRent'] != 0){
									echo "ክራይን መካረይን"; // Only Rent
								}
								else{
									exit(); // Neither paid
								}
...
<table border="2" class="table table-hover">
    ...
    <tr>
        <td><?php echo htmlspecialchars($yearsString); ?></td>
        <td><?php echo number_format($row['sumAmount'], 2); ?></td>
        <td><?php echo number_format($row['sumBankInterest'], 2); ?></td>
        <td><?php echo number_format($row['sumPunishment'], 2); ?></td>
        <td><?php echo number_format($row['sumClean'], 2); ?></td>
        <td><?php echo number_format($row['sumTotal'], 2); ?></td>
        <td><?php echo number_format($row['sumRent'], 2); ?></td>
    </tr>
    <tr>
        <td colspan='3' class='red' align='right'>ጠቅላላ ድምር (ብር)</td> 
        <?php 
        $grandTotal = $row['sumTotal'] + $row['sumRent'];
        ?>
        <td colspan='4' class='blue'>
            <?php 
            // Rounded figure to 2 decimals, letters display properly
            echo number_format($grandTotal, 2) . " (" . numberToTigrinya($grandTotal) . ")";
            ?>
        </td>
    </tr>
</table>
            ...
            <p>ስም ተቐባሊ ገንዘብ: <u><b><?php echo htmlspecialchars($row['paidby']); ?></b></u></p>
