...
// Fetch user full name (logged-in user)
...
if (empty($idList)) die("No payment IDs provided.");

// Prepare placeholders for IN clause
$placeholders = implode(',', array_fill(0, count($idList), '?'));

// SQL query: fetch all approved payments
$sql = "
SELECT 
...
$refs = [];
foreach ($idList as $k => $v) $refs[$k] = &$idList[$k];
call_user_func_array([$stmt, 'bind_param'], array_merge([$types], $refs));

$stmt->execute();
$result = $stmt->get_result();

// Group payments by homeowner_id and payment_date
$grouped = [];
while ($row = $result->fetch_assoc()) {
    $clientid = $row['clientid'];
    $date = $row['payment_date'];
    $grouped[$clientid][$date][] = $row;
}
?>
...
<?php $i = 0; ?>
<?php foreach ($grouped as $clientid => $dates): ?>
    ...
    <?php foreach ($dates as $payment_date => $records): ?>
        ...
        foreach ($records as $r) {
            $years[] = $r['payment_month'];
            $amounts[] = $r['amount_paid'];
            $methods[] = $r['payment_method'];
            $tins[] = $r['tin'];
            $receipts = $r['receipt_no'];
            $decidedbys[] = $r['decidedby'];
			$paidby =$r['paidby'];
        }
        ...
        <div id="printableDiv" class="panel-body"><br>
            <form class="form-horizontal" role="form">
                <fieldset>
                    ...
                    <p align='right'>ደረሰይ ቁፅሪ: <b><?php echo htmlspecialchars($receipts); ?></b></p>
                    <p align='right'>ዕለት: <b><?php echo htmlspecialchars($payment_date); ?></b></p>
                    <p>ሙሉእ ሽም ተከባሊ: <u><b><?php echo htmlspecialchars($fullname); ?></b></u></p>
                    <p>ምክንያት ክፍሊት: <b>ክራይን መካረይን</b></p>
<?php
...
?>
                    ...
                    <p>ስም ተቐባሊ ገንዘብ: <u><b><?php echo htmlspecialchars($paidby); ?></b></u></p>
