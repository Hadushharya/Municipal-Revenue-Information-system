...
@media print {
    ...
    @page {
        size: auto;
        margin: 1mm; /* Reduce default margins to prevent extra page */
    }
}
</style>
...
$userid = $_GET['receiver_id_number'];
$id1 = $_GET['id'];

$stmtd = $conn->prepare("SELECT * FROM `clientslandtransactionpayments` WHERE forcashierstatus='approved' AND id = ? AND receiver_id_number = ?");
$stmtd->bind_param("ii", $id1, $userid);
$stmtd->execute();
$resultd = $stmtd->get_result();

while ($rowd = $resultd->fetch_assoc()) {
    $iddd = $rowd['id'];
    $paidtime1 = $rowd['paidtime'];
          $fullname= $rowd['receiver_fullname'];
		  $receiver_id_number= $rowd['receiver_id_number'];
		  $filenumber= $rowd['filenumber']; 
		   $transaction_type=$rowd['transaction_type'];
		   $amount=$rowd['amount']; 
		   $paymentby= $rowd['paymentby'];
		   $registered_by= $rowd['registered_by'];
		 $receiptnumber=  $rowd['receiptnumber'];
...
?>
					...
					<!-- Print button -->
					<div class="no-print" style="margin-bottom: 1px;">
						<button onclick="window.print();" class="btn btn-primary">Print Receipt</button>
					</div>
					<div id="printableArea">
					<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
										 <fieldset>
										 <legend><u><b>ናይ ክፍሊት ቅብሊት ቅጥዒ</b></u> </legend>
										  <p align='right'>ደረሰይ ቁፅሪ: <?php echo '<b>'.$receiptnumber.'</b>';?></p>
										   <p align='right'>ዕለት: <?php echo '<b>'.$paidtime1.'</b>';?></p>
										     
										
										 <p>ሙሉእ ሽም ከፋሊ ግብሪ: <?php echo '<u><b>'.$fullname.'</b></u>';?></p>
										  <p>ከፋሊ ግብሪ ፋ/ቁፅሪ: <?php echo '<b>'.$filenumber.'</b>';?></p>
										  <p>ምክንያት ክፍሊት: <?php echo '<b>'.$transaction_type.'</b>';?></p>
										<?php
...
?>
      <table border="2"class="table table-hover" >
		...
		<?php function calculateEthiopianYearDifference() {
    ...
    $currentyear1 = calculateEthiopianYearDifference();?>
		...
		<tr> <td colspan='3' align='right' class='red'> ድምር(ብር): </td><td colspan='2' align='left' class='blue'>  <?php  $total=$amount; echo number_format($total, 2) . " (" . numberToTigrinya($total) . ")";?></td>	</tr>
        
      </table>
	  <p>
		 
		ስም ተቐባሊ ገንዘብ: <?php echo '<u><b>'.$registered_by.'</b></u>';?>
