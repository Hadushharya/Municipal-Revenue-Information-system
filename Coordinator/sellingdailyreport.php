					  ...
					  <!-- /.ace-settings-container -->
  					 ...
  					 <div class="col-xs-12" >
                             ...
                             <form class="form-horizontal"  role="form" method="post" enctype="multipart/form-data">
								         ...
								         <label class="col-sm-2 control-label no-padding-right" for="form-field-1">ዕለት:  </label>
										<select  class="col-xs-9 col-sm-2"  id="form-field-1"  required  name="paidtime" id="paidtime">
													<option value=""> ዕለት ምረፅ</option>
							<?php $qrule2 = "SELECT DISTINCT paidtime FROM `clientslandtransactionpayments`  where  forcashierstatus='approved' ORDER BY paidtime DESC";
											$stmt = $conn->prepare($qrule2);
											$stmt->execute();
											$resultr2 = $stmt->get_result();

											while ($rowpaidtime = mysqli_fetch_array($resultr2)){?>
													<option value="<?php echo  $rowpaidtime['paidtime']; ?>"> <?php 
																		 $paidtime=$rowpaidtime['paidtime'];
																		 	 echo $paidtime; 
																		       	 ?></option>
																		 <?php }?> 
												</select>
...
if (isset($_POST['save'])) {
    ...
    $where = "forcashierstatus = 'approved' AND paidtime = ?";
    $params = [$paidtime];
    $types = "s";

    // Add optional paymentby filter
    if (!empty($paymentby)) {
        $where .= " AND paymentby = ?";
        $params[] = $paymentby;
        $types .= "s";
    ...
    $query = "SELECT * FROM clientslandtransactionpayments WHERE $where";
    ...
    echo "<tr style='background-color:#198754; color:white; font-weight:bold;'>
            <td colspan='12' class='text-end'><strong>ድምር ገንዘብ: " . number_format($totalAmount, 2) . " ብር</strong></td>
          </tr>";
      ...
      insertLog($conn, $useridc, $username, "Daily Transaction Report", "Generated", "success", $ip, $agent);
    }
    ?>
  </tbody>
</table>
